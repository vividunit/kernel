			      ...
			      struct snd_soc_dai *dai)
{
	...
	if (es8316->spk_ctl_gpio) {
		if(gpiod_get_value(es8316->hp_ctl_gpio)) {
                	gpiod_set_value(es8316->spk_ctl_gpio, 0);
			//printk("HP Insert!/n");
		} else {
                	gpiod_set_value(es8316->spk_ctl_gpio, 1);
			//printk("HP Not Insert!/n");
		}
	}
			    ...
			    const struct i2c_device_id *id)
{
	...
	es8316->spk_ctl_gpio = devm_gpiod_get_optional(dev,
                                                       "spk-con",
                                                        GPIOD_OUT_LOW);
        if (IS_ERR(es8316->spk_ctl_gpio)) {
                dev_info(dev, "Can't request spk-con gpio\n");
		es8316->spk_ctl_gpio = NULL;
        }

        es8316->hp_ctl_gpio = devm_gpiod_get_optional(dev,
                                                        "hp-det",
                                                        GPIOD_IN);
        if (IS_ERR(es8316->hp_ctl_gpio)) {
                dev_info(dev, "Can't request hp-det gpio\n");
		es8316->hp_ctl_gpio = NULL;
        }
